 ...
 * 숫자들이 들어있는 배열 nums가 매개변수로 주어질 때, nums에 있는 숫자들 중 서로 다른 3개를 골라 더했을 때 소수가 되는 경우의 개수를
            ...
            for (int c = i+1; c < nums.length; c++) {
                for (int j = c+1; j < nums.length; j++) {
                    ...
                    if(isPrimeNumber(checkNum)){
                        System.out.println("다음을 이용하여 >>> "+ nums[i]+" / " + nums[c]+ " / "+ nums[j] + " 소수 " + checkNum +"를 만들 수 있습니다.");
                        answer++;
                    } else {
                        continue;
                    }
