 ...
 * 배열 stages가 매개변수로 주어질 때, 실패율이 높은 스테이지부터 내림차순으로 스테이지의 번호가 담겨있는
...
public class FailureRate {
    ...
    // 92% 커버리지. 13, 22번 테스트 케이스 통과 못함. 이유 알수 없음
    public int[] solution(int N, int[] stages) {
        int totalMembers = stages.length;
        ...
        for (int i = 1; i <= N+1; i++) {
            if(map.get(i) == null){
                ...
                if(i > N){
                    ...
                    failureRate = (int)((float) map.get(i)/(totalMembers-loser) * 1000000000);
                }
                loser += map.get(i);
            }
