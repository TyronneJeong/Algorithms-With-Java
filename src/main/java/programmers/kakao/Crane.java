 ...
 * 각 격자 칸에는 다양한 인형이 들어 있으며 인형이 없는 칸은 빈칸입니다. 모든 인형은
 * "1 x 1" 크기의 격자 한 칸을 차지하며 격자의 가장 아래 칸부터 차곡차곡 쌓여 있습니다.
...
public class Crane {
    ...
    public int solution(int[][] board, int[] moves) {
        ...
        for (int i = 0; i < moves.length; i++) {
            holeNumb = moves[i]-1; // 값에 따라 선택 구멍이 달라진다.
            for (int j = 0; j < board.length; j++) {
                if(board[j][holeNumb] != 0){
                    try {
                        if(st.peek() == board[j][holeNumb]){
                            filterCnt+=2;
                            st.pop();
                            board[j][holeNumb] = 0;
                            break;
                        } else {
                            st.push(board[j][holeNumb]);
                            board[j][holeNumb] = 0;
                        }
